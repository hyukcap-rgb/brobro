...
export default function Matches() {
  ...
  // 공고가 여러 번 반복되는 것처럼 보여 오류로 오인하기 쉬웠다. 화면에서
  // 같은 공고(같은 사이트·공고번호)의 매칭을 한 행으로 묶고, 대표 매칭 1건의
  // 키워드·수량을 보여주되 나머지는 "외N건"으로 표시한다. 저장되는 원본
  // 데이터는 그대로 유지된다(모든 매칭이 그대로 저장된다) — 화면에 보여주는
  // 방식만 바꾼다. 이후(같은 날) 엑셀 다운로드·메일 발송에도 같은 규칙을
  // 적용했는데, 그건 서버 쪽 데이터(매칭마다 별도 행)를 다루는 별도 코드라
  // api-server/src/lib/matches-store.ts의 groupMatchesByNotice()에 똑같은
  // 로직을 따로 구현해뒀다 — 프런트/백엔드가 서로 다른 패키지라 공유 유틸
  // 없이는 합칠 수 없어서다(2026-09-24 최적화 검토 시 확인, 두 곳 다 작고
  // 안정적인 로직이라 별도 공유 패키지를 새로 만들 정도는 아니라고 판단).
