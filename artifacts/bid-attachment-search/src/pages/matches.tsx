    ...
    <div className="mx-auto max-w-7xl p-6 space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {/* 요구사항(2026-09-11 사용자 지적: 나라장터 화면 캡처 — "검색유형"에
          이미 "최종낙찰자"가 있고 그 탭도 개찰일자 기준으로 조회한다며 "최종낙찰자가
          있는 건 중 개찰일을 기준으로 검색하면 모든게 해결되잖아... 모든것의
          기준이야"): 나라장터 공식 화면의 "최종낙찰자" 검색 유형과 동일하게, 지정한
          기간의 개찰일 기준으로 조회한 뒤 그중 낙찰자가 확정된 공고만 서버에서
          걸러서 돌려준다(daily-scan.ts executeScanRun 참고). "지금 실행"과
          동일한 기준이며 더 이상 낙찰일(확정일) 기준으로 넓게 훑지 않는다. */}
          <div className="rounded-md border bg-muted/30 p-3 space-y-2">
            <div className="text-sm font-medium">기간 지정 재검색 (개찰일 기준, 최종낙찰자만)</div>
            <p className="text-xs text-muted-foreground">
              위 "지금 실행"과 별개로, 지정한 기간의 개찰일을 기준으로 조회하되 그중 낙찰자가
              확정된 공고만 검색합니다(시작일=종료일이면 하루만 검색. 아직 낙찰자가 확정되지
              않은 건은 결과에서 제외됩니다).
            </p>
