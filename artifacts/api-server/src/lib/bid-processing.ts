...
//
// 요구사항(2026-09-24 사용자 리포트, 화면 캡처로 확인: "현장 주소"에 "에
// 종사하는 노무자와 통행인의 안전을 위하여 교통안내시설물을 필히
// 설치하여야한다"처럼 주소가 아닌 문장이 그대로 표시됨): 원인은 라벨과
// 값 사이의 구분자(":"/"："/"|")를 선택(?) 처리해서, 첨부파일 본문에
// "공사장소에 종사하는 노무자와 통행인의 안전을 위하여..."처럼 라벨
// 단어가 평범한 문장 중간에(구분자 없이) 나오기만 해도 그 뒤 문장을
// 그대로 주소로 잘못 캡처했기 때문이다(실제 재현: "공사장소" 뒤에 곧바로
// "에 종사하는 ..."가 이어지는 안전관리 문구). 두 가지로 막는다:
// 1) 구분자를 필수로 바꿔 라벨이 단순히 문장 속에 등장하는 경우를 제외하고,
// 2) 그래도 남을 오탐에 대비해 캡처한 값이 최소한 한국 주소처럼 보이는지
// (시/군/구/읍/면/동/리/로/길로 끝나는 덩어리가 하나라도 있는지) 검증한다.
