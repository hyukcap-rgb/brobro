...
export async function ensureSchema(): Promise<void> {
  await pool.query(`
    ...
    -- 요구사항(2026-09-22 사용자가 brog2b.com 실제 화면에서 문제 확인 후 삭제
    -- 요청): daily-scan.ts의 "수량 없는 키워드는 제외" 필터가 (1) 단위 셀만
    -- 단독 매칭되어 수량 숫자 없이 "M2"처럼 단위만 quantity_text에 남거나,
    -- (2) 수량이 문자 그대로 "0"이어서 "0 ㎡"처럼 저장되는 두 경우를 놓쳤던
    -- 버그(위 daily-scan.ts 수정으로 원인 자체는 막음)로 이미 쌓인 기존 행을
    -- 정리한다. 예: "2026년 긴급누수복구공사 연간단가계약(북구3권역)2차" 공고
    -- 하나가 "부직포 · 0 m²" 행 24건으로 목록을 채우고 있던 경우. 조건은
    -- daily-scan.ts의 새 필터(quantityValue가 없거나 0 이하면 제외)와
    -- 동일하게 맞춘다 — quantity_text가 숫자로 시작하지 않거나(단위만 있음),
    -- 맨 앞 숫자가 0인 행. daily-scan.ts 수정 이후로는 이 조건에 해당하는 새
    -- 행이 생기지 않으므로 매 부팅 재실행해도 안전하다(멱등, 재실행 시
    -- 0건 삭제).
    DELETE FROM awarded_matches
      WHERE quantity_text IS NOT NULL
        AND (
          quantity_text !~ '^[0-9]'
          OR COALESCE(
               NULLIF(regexp_replace(substring(quantity_text FROM '^[0-9,]+'), ',', '', 'g'), '')::numeric,
               0
             ) <= 0
        );
    ...
    -- 요구사항(2026-09-24 사용자 리포트, 화면 캡처로 확인: "현장 주소"에
    -- "에 종사하는 노무자와 통행인의 안전을 위하여 교통안내시설물을 필히
    -- 설치하여야한다"처럼 주소가 아닌 문장이 그대로 표시됨): 원인은
    -- bid-processing.ts의 extractSiteAddress가 라벨("공사장소" 등)과 값
    -- 사이의 구분자(":"/"："/"|")를 선택 처리해서, 첨부파일 본문에 라벨
    -- 단어가 구분자 없이 평범한 문장 속에 나오기만 해도 그 뒤 문장을 그대로
    -- 주소로 잘못 캡처한 것(위 bid-processing.ts 수정으로 원인 자체는 막고,
    -- 캡처값이 실제 주소처럼 보이는지도 검증하도록 함). 이미 이렇게 저장된
    -- 기존 행은 site_address를 NULL로 되돌린다 — 화면은 site_address가
    -- NULL이면 "주소 미확인"(또는 사업자 주소)으로 대체 표시하므로, 엉뚱한
    -- 문장을 보여주는 것보다 안전하다. 조건은 위 수정의 검증 로직과 동일하게
    -- "시/군/구/읍/면/동/리/가/로/길로 끝나는 한국 주소 덩어리가 하나도 없는
    -- 값"으로 판단한다. 수정 이후로는 이 조건에 해당하는 새 행이 생기지
    -- 않으므로 매 부팅 재실행해도 안전하다(멱등, 재실행 시 0건 갱신).
    UPDATE awarded_matches
      SET site_address = NULL
      WHERE site_address IS NOT NULL
        AND site_address !~ '[가-힣0-9]{1,10}(시|군|구|읍|면|동|리|가|로|길)(?![가-힣0-9])';

    -- 요구사항(2026-09-26 사용자 리포트, 화면 캡처로 확인: 위 정리 이후에도
    -- "이외의 외부로 유출되는 일이 없...", "의 상태가 설계서와 다를 때"처럼
    -- 여전히 문장이 "현장 주소"에 표시됨): 원인은 실제로 콜론이 있는
    -- 경우였다 — "작업장소 : 이외의 외부로 자료가 유출되는 일이 없도록
    -- 한다"처럼 보안각서/시방서의 조항 제목이 라벨 단어와 우연히 같고
    -- 콜론 뒤에 조항 본문이 이어지는 문서였다. 위 정리가 "시/군/구/읍/면/동/
    -- 리/가/로/길 덩어리가 하나라도 있으면 주소로 인정"했는데, "외부로"의
    -- "로", "상태가"의 "가"가 조사로 우연히 걸려 통과해버렸다(bid-processing.ts
    -- 수정과 동일하게, 진짜 주소는 이런 덩어리가 보통 2개 이상 연달아
    -- 나온다는 점을 이용해 2개 미만이면 정리한다). 수정 이후로는 이 조건에
    -- 해당하는 새 행이 생기지 않으므로 매 부팅 재실행해도 안전하다(멱등,
    -- 재실행 시 0건 갱신).
    UPDATE awarded_matches
      SET site_address = NULL
      WHERE site_address IS NOT NULL
        AND (
          SELECT count(*) FROM regexp_matches(
            site_address,
            '[가-힣0-9]{1,10}(시|군|구|읍|면|동|리|가|로|길)(?![가-힣0-9])',
            'g'
          )
        ) < 2;
